...
| 15 | (yeni karar — makalede yok) Steering etkisi için **kontrol yönü** raporlanmıyor | **Üç kontrol yönü**: `gaussian` (izotropik rastgele), `shuffled` (eksenin kendi koordinatlarının permütasyonu), `rolespan` (rol vektörlerinin span'inde, eksene ortogonal). Hepsi birim norma normalize edilip eksenle **birebir aynı** büyüklükle ölçeklenir ve **aynı üretim kod yolundan** geçer. Ayrı ön-tescil: `results/control_preregistration.json` | Aşama 4'ün sonucu tek başına "bu iş bu **yöne** özgü" ile "bu **büyüklükte** herhangi bir bozulma yeter" arasını ayıramaz. C kriteri (ölçümden önce): hiçbir kontrol yönünün artışı B'nin 25 puanlık eşiğine ulaşmamalı. Ölçülen: eksen +48.4 · gaussian −14.8 · shuffled −3.6 · rolespan −25.7 puan — üçü de negatif, kriter geçti |
